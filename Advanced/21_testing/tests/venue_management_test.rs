...
fn museum_with_three_paintings() -> Museum {
    ...
    museum.buy_painting("The Storyy Night");
...
#[rstest]
fn veneue_management_interacts_with_museum_venue(museum_with_three_paintings: Museum) {
    let mut veneu_mgmt = VenueManagement::new(museum_with_three_paintings);
    veneu_mgmt.make_money();
    assert_eq!(veneu_mgmt.venue.revenue, 25);
    assert_eq!(veneu_mgmt.venue.paintings.len(), 3);
}

#[rstest]
fn veneue_management_interacts_with_movie_theater_venue(
